...
net1 = cv.dnn.readNet('project_files/yolov8_tiny.weights', 'project_files/yolov8_tiny.cfg')
net2 = cv.dnn.readNet('project_files/yolov8.weights', 'project_files/yolov8.cfg')
weightsPath = 'project_files/yolov8.weights'
configPath = 'project_files/yolov8.cfg'
net22 = cv.dnn.readNetFromDarknet(configPath, weightsPath)
...
cap = cv.VideoCapture("test 2.mp4") 
width  = cap.get(3)
height = cap.get(4)
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        if(len(scores)!=0 and scores[0]>=0.7):
            if((recarea/area)<=0.1 and box[1]<600):
                ...
                cv.putText(frame, "%" + str(round(scores[0]*100,2)) + " " + label, (box[0], box[1]-10),cv.FONT_HERSHEY_COMPLEX, 0.5, (255,255,0), 1)
                if(i==0):
